...
int MHI_AC_Ctrl_Core::loop(uint max_time_ms) {
  ...
  long startMillis = esp_timer_get_time()/1000;             // start time of this loop run
...
static byte MOSI_frame[33];
  ...
  call_counter++;
  int SCKMillis = esp_timer_get_time()/1000;               // time of last SCK low level
  while (esp_timer_get_time()/1000 - SCKMillis < 5) {      // wait for 5ms stable high signal to detect a frame start
